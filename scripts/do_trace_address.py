...
def get_transactions(tids):
    print("GET Transactions " + str(tids))
...
def get_output_addresses(transaction):
    ...
    for l in transaction["vout"]:
        ans += l.get("scriptPublicKey", {}).get("addresses", [])
    return ans
...
def get_input_addresses(transaction):
    ...
    for l in transaction["vin"]:
        if "txid" in l:
            txs_in.add(l["txid"])
    for tx in get_transactions(tx_ins):
        ans += get_output_addresses(tx)
    return ans
...
def analize_transaction(analized_address, transaction):
    ...
    in_ans = dict()
    out_ans = dict()
    in_ans = dict() if analized_address not in output_addresses else input_addresses
    out_ans = dict() if analized_address not in input_addresses else output_addresses
...
def analize_block_by_height(analized_address, block_height):
    print("analizing block: " + str(block_height))
...
def analize_chain(analized_address):
    last = get_block_count()
    return analize_blocks_from_range(analized_address, 0, last-1)

start()
analize_chain("1KennyH9grzif79WbaQDHpqgTnm25j4rRj")
